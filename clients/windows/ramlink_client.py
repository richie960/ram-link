    ...
    try:
        while True:
            ...
            if c=="1":
                ...
                t=time.perf_counter()
                print("PING:",request(s,OP_PING)[2].decode(), "%.3f ms"%((time.perf_counter()-t)*1000))
            elif c in ("3","4","5"):
                ...
                if c in ("3","5"):
                    for o in range(0,size,MAX_BLOCK):
                        n=min(MAX_BLOCK,size-o)
                        request(s,OP_WRITE,o,n,pattern[o:o+n])
                if c in ("4","5"):
                    out=bytearray()
                    for o in range(0,size,MAX_BLOCK):
                        n=min(MAX_BLOCK,size-o)
                        out.extend(request(s,OP_READ,o,n)[2])
                    if c=="5":
                        print("INTEGRITY:", "PASS" if hashlib.sha256(out).digest()==hashlib.sha256(pattern).digest() else "FAIL")
            elif c=="6":
                break
    finally:
        s.close()
